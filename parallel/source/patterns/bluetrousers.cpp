...
vector<Pattern_Result> Blue_Trousers::start_search(Mat image) {
 ...
 int apature = 8*thickness;
  if(apature % 2 == 0) {apature += 1;}
 

  //-- remove small elements and then blur nearby elements together
  GaussianBlur(blue_trousers, blue_trousers, Size(apature,apature), 0);
  blue_trousers = blue_trousers > 180;
  GaussianBlur(blue_trousers, blue_trousers, Size(2*apature+1,2*apature+1), 0);
